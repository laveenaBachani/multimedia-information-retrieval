...
def task1():
    ...
    matrix_fileName = "adjMatrix_visDesc_k7.npy"
        ...
        def simMatrixSlice(i):
            ...
            for j in range(i + 1):
                ...
                for m in models:
                    ...
                    simScore += algotype(m)(x, y)[0][0] # for use w/ sklearn metrics
                    # simScore += algotype(m)(ans[j, ptr:ptr + featTypeLength], ans[i, ptr:ptr + featTypeLength])[0][0]
                    ptr += featTypeLength

                simMatrix[i, j] = simScore / 10.0

        Parallel(n_jobs=1, backend='loky')(delayed(simMatrixSlice)(i) for i in range(num_images))
